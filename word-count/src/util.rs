...
pub fn open_io_async(conf: &Config) -> (Pin<Box<dyn AsyncRead + Send>>, Pin<Box<dyn AsyncWrite + Send>>) {
    ...
    /*
            ...
            //let mut file = Box::pin(File::open(filename).expect("Unable to open file"));

            let byte_stream = futures::stream::unfold(file, |mut file| async move {
